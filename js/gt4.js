...
var normalizeDomain = function (domain) {
    // 特殊域名示例: uems.sysu.edu.cn/jwxt/geetest/
...
var jsonp = function (domains, path, config, callback) {
    ...
    // 处理jsonp回调,确保每个不同的jsonp都有唯一的回调函数
    var handleCb = function (cbName, data) {

        // 保证只执行一次,所有超时的情况下不会再触发
        if (data.status == 'success') {
            callback(data.data);
        } else if (!data.status) {
            callback(data);
        } else {
            //接口有返回,但返回了错误状态,进入报错逻辑
            callback(data);
        }
